...
function StartRef = refineLeft(start,trace,h, hmin, varargin)
head = 100;
dyn = 0;
if nargin == 5
    dyn = varargin{1};
...
while proc ~= 0
    ...
    temp = ~(traceLong(stop_temp) < hmin) | (slope > 20 | slope < -10 | traceLong(stop_temp) > h );
    %temp = slope > 20 | traceLong(stop_temp) > h; %degree
    
    % no taking over
    checkTemp = start-nbSp;
    logic = checkTemp(1,2:end) >= checkTemp(1,1:end-1);
