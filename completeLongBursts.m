function [burstStart, burstStop] = completeLongBursts(start_temp, stop_temp, spikeTimes)
...
meanS = mean(S);
burstStart = startDirection(startLong, stopLong, spikeTimesLong, S, meanS, winLen);
...
thresh = median(diff(spikeTimes(a:b)));
end



function start = startDirection(start_temp, stop_temp, spikeTimes, S, meanS, win)
...
while summe ~= 0
    ...
    temp = spikeTimes(start_temp-(count-1))-(spikeTimes(start_temp-count)) < alpha.*S;
