...
function multiVisinOne_3(spikeTimes,burstStart,burstStop, shortStart,shortStop, aggStart, aggStop)
...
%%agglomerates
aggStempStart = floor(spikeVec(aggStart).*12500);
aggStempStop = ceil(spikeVec(aggStop).*12500);

width = (aggStempStop-aggStempStart);
x = aggStempStart;
nbBurst = length(x);
...
for b = 1:nbBurst
%    sprintf('%i',b)
    rectangle('Position', [x(b),y(b),width(b), height(b)], 'EdgeColor', 'blue');
end
