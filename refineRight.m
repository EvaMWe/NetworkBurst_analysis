...
function StartRef = refineRight(stops,trace,h, hmin, varargin)
head = 100;
dyn = 0;
...
while proc ~= 0    
    ...
    temp = ~(traceLong(stop_temp) < hmin) & (slope < -10 | traceLong(stop_temp) > h); %degree
    
    % no taking over
    checkTemp = stops+nbSp;
    logic = checkTemp(1,1:end-1) >= checkTemp(1,2:end);
